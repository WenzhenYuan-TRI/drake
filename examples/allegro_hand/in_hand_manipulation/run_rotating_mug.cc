...
class ConstantPositionInput{
 public:
  ConstantPositionInput(){
    ...
    const std::string HandPath = FindResourceOrThrow("drake/manipulation/"
  ...
  void run_rotate_Y(double rotation_angle) {
    mug_state_.UpdateMugPose(mug_pose_);
    mug_state_.GetYRotatedTargetFrame(
                rotation_angle/180.0*M_PI, &target_fingertip_frame_);
    iniIKtarget();
    std::cout<<"Rotating in one direction \n";
    sleep(10);
  ...
  void run_rotate_Z(double rotation_angle) {
    mug_state_.UpdateMugPose(mug_pose_);
    mug_state_.GetZRotatedTargetFrame(
                rotation_angle/180.0*M_PI, &target_fingertip_frame_);
    iniIKtarget();
    std::cout<<"Rotating in one direction \n";
    sleep(10);
 ...
 private:
  ...
  void iniIKtarget(double target_tor = 5e-3) {
    ...
    for(unsigned int cur_finger=0; cur_finger < 4; cur_finger++){
      ...
      finger_target_transfer = target_fingertip_frame_[cur_finger]
                                * fingertip_offset;
